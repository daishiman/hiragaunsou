...
Claude Code(AIがアプリを作ってくれるツール)に、**プロの開発ノウハウ集(スキル17個)** と **開発を最初から最後まで自動で進める司令塔(エージェント)** を追加するキットです。
...
ダブルクリックの代わりに、コマンドプロンプト(Claude Code の Bash ツール等)からコマンドで実行することもできます。挙動はダブルクリックと完全に同じです。
...
**インストール先を `C:\Users\(あなたの名前)\.claude` 以外にしたい場合**(例: 特定のプロジェクト直下にある `.claude` にだけ導入したい場合)は、このインストーラーはそのフォルダに固定されているため使えません。代わりに `skills`・`agents`・`commands` の中身をそのプロジェクトの `.claude` へ手動でコピーしてください(既存ファイルと重複する場合は、上書き前に別フォルダへコピーしてバックアップしてから行うと安全です)。
...
- 表示が崩れていてもインストール自体は正常に行われます。STEP 4 の動作確認で `/build-app` が出れば問題ありません。

### Q9. セットアップスクリプトが「インストールに失敗しました」と表示する
...
1. `skills` フォルダの中の、**上の表にある17個のフォルダ**
