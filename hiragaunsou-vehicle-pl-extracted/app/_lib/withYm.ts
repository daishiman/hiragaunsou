 ...
 * これ以外の画面 (ダッシュボード・管理系・マイページ等) は ym を読まないため、
 * リンクに付けても無視されるだけでURLが汚れる。付ける先をここで限定する。
 *
 * ホーム(/)も ?ym= を読むが、既定は「まだ締めていない、取込のある最も新しい月」を実データから
 * 決める(app/_lib/workingYearMonth.ts)。作業中の月へ自動で戻れることのほうが大事なので、
 * 他画面からのリンクに ym を引き継がせる対象には入れない。
 */
