import type { ImportSourceType } from "../../src/domain/rules/importSources";
...
type UsageKind = "deficit_factor_analysis" | "factor_analysis_report";
...
export function usageKindLabel(kind: string): string {
  return USAGE_KIND_LABELS[kind as UsageKind] ?? "未対応の利用種別";
}
...
export function sourceTypeLabel(sourceType: string): string {
  return SOURCE_TYPE_LABELS[sourceType as SourceType] ?? "判別できない帳票";
}
 ...
 */
type ImportBatchStatus = "completed" | "failed" | "partial";
...
export function importBatchStatusLabel(status: string): string {
  return IMPORT_BATCH_STATUS_LABELS[status as ImportBatchStatus] ?? "状態を確認できません";
}
