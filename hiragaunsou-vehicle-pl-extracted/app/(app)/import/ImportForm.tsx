 ...
 *
 * 表かカードか (T7 §4-1)。ここで利用者がするのは「4つの帳票の値を列で見比べる」ことではなく、
 * 1つの帳票を開いて「これを入れる」と決めること。よって表にはせずカード(投入口)で並べる。
 * 取込済みファイルの一覧も、1帳票あたり数件で見比べる対象ではないため箇条書きのまま。
  ...
  */
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {/*
        対象年月と「あと何ファイル残っているか」は、帳票の欄を下まで見ていく間ずっと
        見えている必要がある。どちらかが画面から消えると、いま何月分のどのファイルを
        入れているのかが分からなくなる (T7 §2-2)。工程の帯の下に貼る。
      ...
      */}
      ...
      >
        <YearMonthSelect basePath="/import" value={yearMonth} options={selectableYearMonths(25)} />
      </StickyFilterBar>

      {/*
        取込のときに何が起きるかの説明。毎月同じ文章を読む必要は無いので、
        文章はそのままに、知りたい人だけが開く折りたたみへ移した。
        説明は貼り付けない (T7 §2-2「貼らないもの」)。
      */}
        ...
        const body = (
          <>
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              {/* 済んだものは brand、まだのものは無彩。札の見た目は Badge 1箇所で決める */}
              <Badge tone={batches.length > 0 ? "brand" : "neutral"}>
            ...
            {isConflicting ? (
              // 注意の面は共通の AlertPanel (caution) で描く。ここだけ自前の枠を書くと、
              // 同じ「注意」が画面ごとに違う見た目になる。
              <div className="mt-4">
                <AlertPanel
                  tone="caution"
                  title={
              ...
              <input
                ...
                className={`${FILE_FIELD_CLASS} mt-3 disabled:opacity-50`}
              />
            )}
            ...
            {result && !isConflicting && !isChecking ? (
              <div className="mt-2">
                {result.ok ? (
                  ...
                  // 失敗は小さい赤い1行だと見落とされる。共通の面で出す
                  <AlertPanel tone="danger" title={`${result.fileName}: ${result.message}`} />
                )}
