...
export function buildTodoResponse(
  ...
  return {
    ...
    cards: sorted,
    emptyMessage: sorted.length === 0 ? "今月の入力は完了しています" : null,
  };
}

export class GetTodoBoardUseCase {
  constructor(private readonly reviewFlagRepo: ReviewFlagRepository) {}

  async execute(yearMonth: string): Promise<TodoResponse> {
