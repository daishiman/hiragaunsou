...
### 11-11. 本文の幅（画面ごとに `max-w-*` を書かない）

依頼者の指摘（2026-08-09）:

> なぜ左側のスペースじゃなくて右側のスペースが空いているのが気になります。
> このデータ取込に関しては、右と左で切り分けるような空白が右側に存在したりします。**全画面共通でお願いします。**

原因は、各ページが独自に `max-w-3xl` `max-w-5xl` を書いていたこと。同じアプリの中で幅が
画面ごとに違うと、利用者には「壊れている」か「まだ作りかけ」に見える。

**幅は `app/_lib/screens.ts` の `width` 宣言 1箇所で決まる。**

| `width` | 幅 | 対象 |
|---|---|---|
| `wide`（既定、省略時） | 画面いっぱい | 表・一覧・工程の画面。数字の桁が多い表を横スクロールさせないため、使える幅は全部使う |
| `narrow` | `max-w-3xl`（左寄せ） | 読むだけの画面と、1列に並ぶだけの設定フォーム。全幅だと1行が長すぎて読めず、入力欄だけが横に伸びる |

- 実際に当てるのは `AppShell` の `<main>` だけ（`contentWidthClass(pathname)`）。
- `narrow` でも**中央寄せ（`mx-auto`）にしない**。中央寄せにすると、サイドバーを隠したときに本文ごと横へ動いて読む位置が変わる。左端はサイドバーと揃えたまま、余白は右にだけ作る。
- ページ側に `max-w-2xl` 〜 `max-w-7xl` を書くと `tests/lib/contentWidth.test.ts` が落ちる。
- 例外として許すのは**部品の中の1要素**に効く上限だけ（入力欄1つの幅、札の文字数上限、ダイアログの幅）。

**使ってはいけない場合**: 「この画面だけ狭くしたい」。そう思ったら幅ではなく、中身の並べ方（カードを2列にする等）で解く。
